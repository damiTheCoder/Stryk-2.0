      ...
      <SidebarInset className="overflow-y-auto overflow-x-hidden">
        <header className="sticky top-0 z-50 flex h-16 shrink-0 items-center gap-3 bg-white/95 dark:bg-slate-950/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 supports-[backdrop-filter]:dark:bg-slate-950/60 md:h-14">
          <div className="flex items-center gap-3 px-4">
            <SidebarTrigger className="-ml-1 size-9 [&_svg]:size-6 md:[&_svg]:size-6" />
            <Separator
            ...
            <Link href="/" className="flex items-center hover:opacity-80 transition-opacity" title="Go to Home">
              <Image src="/brand-logo.png" alt="Stryk" width={24} height={24} className="size-6 rounded-full border border-black dark:border-black object-contain" />
            </Link>
            <DynamicBreadcrumb />
          </div>
        ...
        <CommandPalette />
        <main className="flex flex-1 flex-col overflow-x-hidden pb-20 md:pb-0">{children}</main>
        <MobileBottomNav />
      </SidebarInset>
    </SidebarProvider>
  )
